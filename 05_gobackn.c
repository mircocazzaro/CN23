 ...
 *
 * This program demonstrates the implementation of the Go-Back-N protocol
...
void mytimer(int num) {
	...
	if (timeout <=seconds) { // if the timeout of the window is elapsed
		timeout = seconds + 4;
	...
	*/
    ...
    if (fd[0].revents == POLLIN) { // Check if the (returned) event is POLLIN (incoming data)
		...
		if(ack_no >= winstart && ack_no< winstart + N){ // Check if the acknowledgment is within the window
			int offset = ack_no - winstart + 1;
			for (j = winstart; j<=ack_no; j++) {
				win[j].is_acked=1;
				winstart++;	
			}
