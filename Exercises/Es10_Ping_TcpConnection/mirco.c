...
int sequence = 0;
void forge_tcp(struct ip_datagram * ip, struct tcp_segment * tcp) {
srand(time(NULL));
sequence = rand();
tcp->s_port = htons(19747);
tcp->d_port = htons(80);
...
int main(){
int t,len;
//unsigned int destip = (88 << 24) + (80 << 16) + (187 << 8) + 84; 
...
while(1){
	...
	if(htons(eth->type)==0x0800) 
		if(ip->proto == 6 )
			if(ntohs(tcp->d_port) == 19747 && ntohs(tcp->s_port) == 80){
				printf("Received TCP Response:\n");
				printf("\n");
            printf("TCP received ports are correctly setted :)\n");
            if (tcp->flags == 18) {
               printf("TCP received has syn + ack :)\n");
               if (ntohl(tcp->ack) == sequence + 1) {
                  printf("TCP received sequence number is okay :)\n");
               }
//               printf(":(  sequencen = %d, ack received = %d\n", sequence, ntohl(tcp->ack));
            }
