	...
	if(htons(eth->type)==0x0800) 
		if(ip->proto == 1 )
			if(icmp->type == 0 ){
				...
				printf("%d.%d.%d.%d: Destination Unreachable: ",*(unsigned char*) &ip->src, *(((unsigned char*) &ip->src) +1), *(((unsigned char*) &ip->src) +2), *(((unsigned char*) &ip->src) +3));
				switch (icmp->code) {
					case 0:
				...
				unsigned char hops = 64 - ip->ttl;
				printf("Number of hops of the reply: %d hops.\n", hops);
				break;
			} 
	}
} 
