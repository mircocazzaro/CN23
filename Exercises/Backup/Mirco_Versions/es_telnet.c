...
while(1){
   ...
   if(htons(eth->type)==0x0800) 
		if(ip->proto == 6 )
			if(ntohs(tcp->d_port) == 51234){
				//printf("Received TCP segment:\n");
            printbuf(buf, 14 );
				printbuf(buf+14,t-14);
				printf("\n");
            if (tcp->flags == 2) {
               printf("TCP received has syn :)\n");
            }

            unsigned int idestip = ip->src;
            unsigned short d_port = tcp->s_port;
            unsigned int ack = ntohl(tcp->seq) + 1;
            unsigned short window = tcp->window;

            bzero(tcp, sizeof(struct tcp_segment));
            bzero(ip, sizeof(struct ip_datagram));
            len = 20;
            forge_ip(ip,idestip,len, 6); 
            forge_tcp(ip, tcp, d_port, htonl(ack), window);
            ...
            printbuf(buf, 14 );
            printbuf(buf+14, 40 );
            t = sendto(s, buf, 14+40, 0 ,(struct sockaddr *) &sll, len);
            if ( t == -1) {perror("sendto Failed"); return 1;}
            printf("%d bytes sent\n",t);
            printf("%d is length", sizeof(struct sockaddr_ll));
	            ...
	            if(htons(eth->type)==0x0800)
		            if(ip->proto == 6 )
                        ...
                        printbuf(buf, 14 );
				            ...
				            printf("\n");
                        if (tcp->flags == 16) {
                           printf("TCP received has only ack :)\n");
                        }
                        else {
