...
unsigned char  mss_win[8] = { 0x02, 0x04, 0x05, 0x90, 0x03, 0x03, 0x00, 0x00};
...
switch(tcb->st){
    ...
    tcb->cong_st = SLOW_START;
#endif  
			prepare_tcp(s,SYN,NULL,0,mss_win,sizeof(mss_win));
			tcb->st = SYN_SENT;	
	...
	case SYN_SENT:	
		if(event == PKT_RCV){
            ...
            opt_len = ((tcp->d_offs_res>>4) - 5)*4;
            while (opt_len > 0) {
               //printf("OPT LEN: %d - CURR OFFS: %d- option found: %d\n", opt_len, offset, tcp->payload[offset]);
          
               if (tcp->payload[offset] == 1) {
                  unsigned char cur_len = 1;
                  opt_len = opt_len - cur_len;
                  offset = offset + cur_len;
               }
               else if (tcp->payload[offset] == 3) {
                  printf("### WINDOW SIZE: %d - WINDOW SCALE: %d - ACTUAL WINDOW: %ld ###\n", htons(tcp->window), tcp->payload[offset + 2], (unsigned int) pow(2, tcp->payload[offset + 2])*htons(tcp->window));
                  opt_len = 0;
                  recv_winsize = tcp->payload[offset + 2];
                  break;
               }
               else {
                  if (tcp->payload[offset] == 4 && tcp->payload[offset+2] == 5) {
                     unsigned char cur_len = 4 + tcp->payload[offset+3];
                     opt_len = opt_len - cur_len;
                     offset = offset + cur_len;
                     printf("qua");
                  }
                  else {
                     unsigned char cur_len = tcp->payload[offset + 1];
                     while (cur_len % 4 != 0) {
                        printf("cur_len = %d\n", cur_len);
                        cur_len++;
                     }
		...
		if (eth->type == htons (0x0806)) {
			...
			if (ip->proto == TCP_PROTO){
			  ...
			  if(i<MAX_FD)
                  ...
                  unsigned int pow = 1; 
                  for (int xyz = 0; xyz< recv_winsize; xyz++) {
                     pow = pow * 2;
                  }

                  pow = pow * htons(tcp->window);

                  printf("### WINDOW SIZE: %d - WINDOW SCALE: %d - ACTUAL WINDOW: %ld ###\n", htons(tcp->window), recv_winsize, pow);
