...
void forge_ip(struct ip_datagram * ip, unsigned char* dst,  unsigned short payloadlen, unsigned char proto, int ttlUser  )
{
ip->ver_ihl = 0x45;
ip->tos = 0;
...
printf("Insert TTL: ");
scanf("%d", &ttlUser);
forge_ip(ip,destip,len, 1, ttlUser); 
	...
	if(htons(eth->type)==0x0800) 
		if(ip->proto == 1 )
			if(icmp->type == 0 ){
				...
				printf("%d.%d.%d.%d: Time exceeded: ",*(unsigned char*) &ip->src, *(((unsigned char*) &ip->src) +1), *(((unsigned char*) &ip->src) +2), *(((unsigned char*) &ip->src) +3));
				switch (icmp->code) {
					case 0:
