...
if((addr->sa_family == AF_INET)){
	...
	if ( s >= 3 && s<MAX_FD){
			...
			while(sleep(10)){
					...
					if(destination_unreachable){ perror("ERROR: destination unreachable"); myerrno = ENETUNREACH; return -1;} 
		...
		if (eth->type == htons (0x0806)) {
			...
			if (ip->proto == TCP_PROTO){
			  ...
			  if(i<MAX_FD)
				...
				if (icmp->type == 3 /*&& (icmp->code == 0 || icmp->code == 1)*/) {	
...
addr.sin_addr.s_addr = inet_addr("89.40.142.15"); //faq
