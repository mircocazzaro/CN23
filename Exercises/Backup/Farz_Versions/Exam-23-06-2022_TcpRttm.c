...
void rtt_estimate(struct tcpctrlblk * tcb, struct txcontrolbuf * node,unsigned int TSecr ){
	if(node->retry==1 && !timeoption){ // If not retransmitted
		printf("\n\n\n\n\n\n TIME OPTION OFF \n\n\n\n\n\n\n");
...
int prepare_tcp(int s, unsigned char flags, unsigned char * payload, int payloadlen,unsigned char * options, int optlen){
	...
	txcb->totlen = payloadlen  +12+ 20+optlen;
	txcb->retry = 0;
	...
	tcp->d_offs_res=(5+3+optlen/4) << 4; 
	tcp->flags=flags&0x3F;
	tcp->urgp=0;
	bzero(tcp->timeopt,12);
	tcp->timeopt[0]=0x08;
	tcp->timeopt[1]=10;
	tcp->timeopt[10]=0x01;
	tcp->timeopt[11]=0x01;
...
void update_tcp_header(int s, struct txcontrolbuf *txctrl){
	...
	if((txctrl->segment->flags & ACK)){
		unsigned int* TSvalptr= (unsigned int*)(txctrl->segment->timeopt+2);
		unsigned int* TSecrptr= (unsigned int*)(txctrl->segment->timeopt+6);
		*TSvalptr=htonl((unsigned int)tick);
		*TSecrptr=htonl(tcb->recent);
		...
		case SYN_SENT:	
			if(event == PKT_RCV){
				if((tcp->flags&SYN) && (tcp->flags&ACK) && (htonl(tcp->ack)==tcb->seq_offs + 1)){
					...
					tcb->st = ESTABLISHED;
					int opt_len=((tcp->d_offs_res>>4)*4)-20;
					
					if(opt_len){
						if((unsigned char*)-1 == findoption(((unsigned char*)tcp)+20,opt_len)) timeoption=0;
					}else{
						timeoption=0;
					}
					...
					if (eth->type == htons (0x0806)) {
						...
						if (ip->proto == TCP_PROTO){
							...
							if(i<MAX_FD)
							{
								...
								printf("\n---------------- %d -------------------- \n",opt_len);
								printbuf(((unsigned char*)tcp)+20,opt_len);
								printf("\n------------------------------------ \n"); 
								...
								unsigned int *TSval=NULL,*TSecr=NULL;

								if(timeoption){
									unsigned char* opt=findoption((((unsigned char*)tcp)+20),((tcp->d_offs_res>>4)*4)-20);
									TSval=(unsigned int*) (opt+2);
									TSecr=(unsigned int*) (opt+6);
								...
								if( stream_offs<=tcb->last_ack_sent && tcb->last_ack_sent< (stream_offs+streamsegmentsize)){
									if(timeoption) tcb->recent=htonl(*TSval);
								}
							//	printf("\n\n-----fuori---\n");
								if(tcb->txfirst !=NULL){
									shifter = htonl(tcb->txfirst->segment->seq);
									;//printf("Processing ack  %d\n", htonl(tcp->ack)-tcb->seq_offs);i
								//	printf("\n----------------HERE\n");
										...
										//	printf("QUA\n");
											...
											if(htonl(tcp->ack)-shifter ==(htonl(temp->segment->seq)-shifter + temp->payloadlen)) // Exact ACK matching: estimates
												if(temp->payloadlen!=0) // if not a piggybacked ACK of an ACK 
													...
													if(TSecr!=NULL)	rtt_estimate(tcb,temp,htonl(*TSecr));
													else rtt_estimate(tcb,temp,0);
