...
int main(){
	...
	for(int k=255; k>=0; k--){
		...
		len = forge_icmp(icmp);
		forge_ip(ip,destip,len, 1,k); 
		forge_eth(eth,destmac,0x0800);

		//ERRORE:     ip->ttl=k;//modifying the ttl
		...
		while(1){
			...
			if(htons(eth->type)==0x0800){
				if(ip->proto == 1 ){
					if(icmp->type == 0 ){
						printf("Received Ping Response TTL:%d \n",k);
						//printbuf(buf,t);
						break;
					} else if( icmp->type ==11){
						printf("TIME EXCEEDED %d \n", k);
						printf("IP: %d.%d.%d.%d \n ", ((unsigned char*)(ip->src))[0],((unsigned char*)(ip->src))[1],((unsigned char*)(ip->src))[2],((unsigned char*)(ip->src))[3]);
						return 0;
					}
				}
			}
