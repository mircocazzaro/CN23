...
int forge_icmp(struct icmp_packet * icmp){
	...
	icmp->id = 0x1234;
	icmp->seq = 0;
	for(i=0;i<8;i++) icmp->data[i]=i;
	icmp->checksum = htons(checksum(icmp,16));
	return 16;
}
...
void forge_ip(struct ip_datagram * ip, unsigned char* dst,  unsigned short payloadlen, unsigned char proto  )
{
	...
	ip->id = 0xABCD;
	ip->flags_offs = htons(0x0200); //more fragments
	ip->ttl = 128;
	ip->proto = proto;
	ip->checksum = 0;
...
int forge_payload(unsigned char* payload){
	for(int i=8;i<32;i++) payload[i-8]=i;
	return 24;
}


void forge_ip2(struct ip_datagram * ip, unsigned char* dst,  unsigned short payloadlen, unsigned char proto  )
{
	ip->ver_ihl = 0x45;
	ip->tos = 0;
	ip->len = htons(payloadlen+20);
	ip->id = 0xABCD;
	ip->flags_offs = htons(0x0002);
	ip->ttl = 128;
	ip->proto = proto;
	ip->checksum = 0;
	ip->src = *(unsigned int *)myip;
...
int main(){
	...
	printbuf(buf, 14 + 20 + 16 );
	t = sendto(s, buf, 14+20+16, 0 ,(struct sockaddr *) &sll, len);
	if ( t == -1) {perror("sendto Failed"); return 1;}
	printf("%d bytes sent\n",t);
	...
	bzero(buf,1500);	
	len = forge_payload(ip->payload);
	forge_ip2(ip,destip,len, 1);

	forge_eth(eth,destmac,0x0800);

	printbuf(buf, 14 + 20 + 24 );
	t = sendto(s, buf, 14+20+24, 0 ,(struct sockaddr *) &sll, len);
	if ( t == -1) {perror("sendto Failed"); return 1;}
	printf("%d bytes sent\n",t);
