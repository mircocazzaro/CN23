/*
   1)
   -add to the state variables the last valid tsval received and the last ack sent
   -in the timer (update_tcp_header) just when sending a packet check if new data is being acked and in that case add the option properly encoded
   -in myio when receiving a new segment compare the last_ack sent and the sequence number and length to update the tsval received
   -change the rtt_e estimation  using the option time and our clock time.
 */
...
struct tcpctrlblk{
	...
	unsigned int cgwin;
	unsigned int lta;
	unsigned int last_ack_sent;
	unsigned char TSReceived;
#endif
};
...
void rtt_estimate(struct tcpctrlblk * tcb, struct txcontrolbuf * node,unsigned int tx_time ){
	if(node->retry==1){ // If not retransmitted
		int rtt = tick -( tx_time? tx_time: node->txtime);
...
void update_tcp_header(int s, struct txcontrolbuf *txctrl){
	...
	if(tcb->st == ESTABLISHED){
		struct TSOpt* rttmopt=(struct TSOpt*)txctrl->segment->payload;
		rttmopt->kind=8;
		rttmopt->len=10;
		unsigned int* tmpo= (unsigned int*) rttmopt->TSeen;
		unsigned int* tmpo2= (unsigned int*) rttmopt->TSval;
		*tmpo2=htonl((unsigned int)tick);

		if((txctrl->segment->flags&ACK)){
			tcb->last_ack_sent=tcb->cumulativeack;
			*tmpo=htonl(tcb->TSReceived);
		}else{
			*tmpo=0;
		}
...
int mywrite(int s, unsigned char * buffer, int maxlen){
	...
	for(j=0;j<actual_len; j+=fdinfo[s].tcb->mss){
		...
		struct TSOpt* rttmopt= (struct TSOpt*) malloc(sizeof(struct TSOpt));
		prepare_tcp(s,ACK,buffer+j,len,(unsigned char*) rttmopt,10);
					...
					if (eth->type == htons (0x0806)) {
						...
						if (ip->proto == TCP_PROTO){
							...
							if(i<MAX_FD)
							{
								...
								unsigned int peer_time=0;
								unsigned int prev_time=0;
								
								unsigned int opt_len=((tcp->d_offs_res>>4)*4) - 20;
								unsigned char* options= ((unsigned char*)ip)+20;
								int idx=0;
					
								printf("------------------HERE HERE %d---------",opt_len); 
								
								if((stream_offs<= tcb->last_ack_sent) && (tcb->last_ack_sent<(stream_offs+streamsegmentsize)) ){
									...
									while(idx<opt_len){
										switch(options[idx]){
											...
											case 1:
												...
												break;
											case 8:
												prev_time=htonl(options[idx+6]);
												peer_time=htonl(options[idx+2]);
												printf(" \n\n OUR: %d \n\n PEER: %d \n\n", prev_time,peer_time);
												idx+=10;
												break;
											default:
									...
									if(peer_time) tcb->TSReceived=peer_time;
											...
											if(htonl(tcp->ack)-shifter ==(htonl(temp->segment->seq)-shifter + temp->payloadlen)) // Exact ACK matching: estimates
												if(temp->payloadlen!=0) // if not a piggybacked ACK of an ACK 
													if(temp->retry<=1) // if never retransmitted or no other segment in the window has been retransmitted.  
														rtt_estimate(tcb,temp,prev_time);
