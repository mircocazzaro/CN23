...
void forge_ip(struct ip_datagram * ip, unsigned char* dst,  unsigned short payloadlen, unsigned char proto,unsigned char ttl  ) {
	ip->ver_ihl = 0x45;
	ip->tos = 0;
...
//unsigned char destip[4] = {88,80,187,124};
	unsigned char destip[4] = {8,8,8,8};
...
//bzero(&sll,sizeof(struct sockaddr_ll));
				...
				//if(test) {
					if(ip->proto == 1 ) {
						if(icmp->type == 0 ) {
							...
							printf("%d HOST: %d.%d.%d.%d answered \n",j,((unsigned char*) &(ip->src))[0],((unsigned char*) &(ip->src))[1],((unsigned char*) &(ip->src))[2],((unsigned char*) &(ip->src))[3]);
							break;
						}
					}
				//}else{
	...
	if(j==256) printf("Error it was not possible to reach the end host \n");
