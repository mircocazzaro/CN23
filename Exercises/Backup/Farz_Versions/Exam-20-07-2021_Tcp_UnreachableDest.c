			...
			if (fds[0].revents & POLLIN){
				...
				while ( 0 <= (size = recvfrom(unique_s,eth,MAXFRAME,0, (struct sockaddr *) &sll,&len))){
					...
					if (eth->type == htons (0x0806)) {
						...
						}else if(ip->proto==1){//If TCP protocol
							//printf("\n -----------------------ICMP RECEIVED----------------------------------\n");
							struct icmp_packet* icmp=(struct icmp_packet*) ((char*)ip + ((ip->ver_ihl&0x0F)*4)); // getting the returned icmp packet

							struct ip_datagram* oldip=(struct ip_datagram*) (((char*) icmp) + 8); // getting the returned ip packet

							struct tcp_segment * tcp = (struct tcp_segment *) ((char*)oldip + (oldip->ver_ihl&0x0F)*4); //getting the old TCP packet

							if(icmp->type == 3){
								//printf("\n ------------TYPE 3-------------\n");
								for(i=0;i<MAX_FD;i++)
									if((fdinfo[i].st == TCB_CREATED) && (fdinfo[i].l_port == tcp->s_port) 
										&& (tcp->d_port == fdinfo[i].tcb->r_port) && (oldip->dstaddr == fdinfo[i].tcb->r_addr))
										break;
								if(i<MAX_FD){
									//printf("\n\nsocket found \n\n\n");
									fdinfo[i].tcb->st=TCP_CLOSED;
									fdinfo[i].tcb->refused=1;
								}
