/*
   ...
   Implementation Date: 21/6/23
 
# IDEA #
- We have to introduce the MSS management for both client and server   
- We modify the 'fsm' function to check if we received the MSS option in 
   > syn+ack
   > syn
- if we received, we set the mss in the tcb appropriately
- We modify the 'mytimer' function to print details about the out-going segments
- We modify the 'myio' function to print details about the in-going segments
...
#define OPTTYPE 2
unsigned char* findoption(unsigned char* opts, int len) {
   int i = 0;
   while(i<len) {
      switch(opts[i]) {
         case 0:
	...
	case SYN_SENT:	
		if(event == PKT_RCV){
			if((tcp->flags&SYN) && (tcp->flags&ACK) && (htonl(tcp->ack)==tcb->seq_offs + 1)){
				// ##
            unsigned char* opts = ((unsigned char*)tcp)+20;
            int optlen = ((tcp->d_offs_res>>4)*4)-20;

            if(optlen == 0) {
               printf("\n######## NO OPT FOUND: CANNOT SET MSS ########\n");
            } else {
               unsigned char* rcvmssopt = findoption(opts, optlen);
               if(rcvmssopt == (unsigned char*)-1) {
                  printf("\n######## NO MSS OPT AMONG OPTIONS ########\n");
               }
               else {
                  unsigned short* mss = (unsigned short*)(rcvmssopt+2);
                  printf("\n######## MSS OPT FOUND! VALUE: %u ########\n", htons(*mss));
                  tcb->mss = htons(*mss);
               }
            }
...
for(i=0;i<MAX_FD;i++){
	if(fdinfo[i].st == TCB_CREATED){
		...
		for(tot=0,txcb=tcb->txfirst;  txcb!=NULL  /*&& (tot<tcb->radwin)*/;  txcb = txcb->next){
         ...
         // ##
         printf("\n## (OUT ");
         if((txcb->segment->flags) & SYN) {
            printf("SYN");
            if((txcb->segment->flags) & ACK)
               printf("+ACK");
         } else if((txcb->segment->flags) & FIN) {
            printf("FIN");
            if((txcb->segment->flags) & ACK)
               printf("+ACK");
         } else if((txcb->segment->flags) & ACK) {
            printf("ACK");
         }
         printf(" %u) ##\n", txcb->payloadlen);
