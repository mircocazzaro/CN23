...
int forge_icmp(struct icmp_packet * icmp, unsigned int len, unsigned int offset){
int i;
icmp->type = 8;
icmp->code = 0;
icmp->checksum = 0;
icmp->id = 0x1234;
icmp->seq = 0;
for(i=0;i<len;i++) icmp->data[i]=i+offset;
icmp->checksum = htons(checksum(icmp, 8+len));
return 8+len;
}
...
printf("\nFIRST SEND payload:");
printbuf(buf+14+20+8, 16);
//printbuf(buf, 14 + 20 + 8+16);
t = sendto(s, buf, 14+20+8+16, 0 ,(struct sockaddr *) &sll, len);

memset(ip, 0, sizeof(struct ip_datagram));
memset(icmp, 0, sizeof(struct icmp_packet));
len = forge_icmp(icmp, 32-16, 16);
forge_ip(ip, destip, len, 1);

printf("\nSECOND SEND payload:");
printbuf(buf+14+20+8, 32-16);
//printbuf(buf, 14 + 20 + 8+(32-16));
t = sendto(s, buf, 14+20+8+(32-16), 0 ,(struct sockaddr *) &sll, len);
