/*
1)
-add the sack permit option to the mssoption (to sent it in the initial SYN segment)
-create an array of 40 bytes representing the sack option
-add a field in the txcontrolbuf structure to be used as the ACK'd flag
-in myio, when I receive the packets, I will check (based on the ack received, after the cycle that removes the acknowledged transmitted segments) which segments have been ACKED using the sack option.
-in mytimer i will check the new flag (on the txcontrolbuf structure) to determine if i need to retransmit the segment or not.
-I will add in the prepare_tcp calls in the write method the sackoption
-in update_tcp_header I will add a cycle to fill the option based on the unack queue

2)
-my logic considers both the reception and the transmission of the sack options
-if no option is received the flags are not updated and the tcp will work in the same way as a normal tcp program 
-my timer checks the flags for retransmission
-in update_tcp_header we "create" the blocks to be put in the option by considering the consiguous blocks.
...
unsigned char  mssopt[8] = { 0x02, 0x04, 0x05, 0x90,0x04,0x02,0x00,0x00};
unsigned char  sackopt[40];
...
unsigned char* findoption(unsigned char* opt, int opt_len){
	int idx=0;
	while(idx<opt_len){
		switch(opt[idx]){
			case 0:
...
void update_tcp_header(int s, struct txcontrolbuf *txctrl){
	...
	if(tcb->st==ESTABLISHED){
		bzero(txctrl->segment->payload,40);
		unsigned char* sackopt2=txctrl->segment->payload;
		unsigned short relorig=0;
		unsigned short blksize=0;
		unsigned short* relptr;
		unsigned short* szptr;
		int blkcount=0;
		struct rxcontrol* unack=tcb->unack;
		while(unack!=NULL && blkcount<9){
			relorig=unack->stream_offs-tcb->cumulativeack;
			blksize=unack->streamsegmentsize;
			while(unack->next!=NULL && unack->next->stream_offs==(unack->stream_offs+unack->streamsegmentsize)){
				unack=unack->next;
				blksize+=unack->streamsegmentsize;
			}
			relptr=(unsigned short*) (sackopt2+2+4*blkcount);
			szptr=relptr++;
			*relptr=htons(relorig);
			*szptr=htons(blksize);
...
int mywrite(int s, unsigned char * buffer, int maxlen){
	...
	for(j=0;j<actual_len; j+=fdinfo[s].tcb->mss){
		...
		prepare_tcp(s,ACK,buffer+j,len,sackopt,40);
			...
			for(tot=0,txcb=tcb->txfirst;  txcb!=NULL && (tot<(tcb->cgwin+tcb->lta)); tot+=txcb->totlen, txcb = txcb->next){
				if(txcb->retry==0) //first transmission
					...
					for(tot=0,txcb=tcb->txfirst;  txcb!=NULL  /*&& (tot<tcb->radwin)*/;  txcb = txcb->next){
						...
						if(txcb->txtime+tcb->timeout > tick || txcb->ackd )  continue; //No timeout or packet is already acked
...
#endif
										unsigned int ack_no=htonl(tcp->ack);
										unsigned char* rxopt=findoption(((unsigned char*)tcp)+20,(((tcp->d_offs_res>>4)*4)-20));
										if(rxopt != (unsigned char*)-1){
											int total=(int)rxopt[1];
											int blocks=total/4;

											printf("\n ------------%d-----------\n",blocks);
											printbuf(rxopt,rxopt[1]);									
											printf("\n -----------------------\n");
											struct txcontrolbuf* tmp=NULL;
											for(int i=0; i<blocks; i++){
												tmp=tcb->txfirst;
												unsigned short* relorig=(unsigned short*)(rxopt+2+(blocks*4));
												unsigned short* blksz=relorig++;
												//printf("\n relorig: %d \n blksz: %d\n",relorig,blksz); 
												while(tmp != NULL){
													if(htonl(tmp->segment->seq)>=(ack_no+(htons(*relorig))) && (htonl(tmp->segment->seq)+tmp->payloadlen)<=(ack_no+(htons(*relorig))+(htons(*blksz)))){
														tmp->ackd=1;
													}
													tmp=tmp->next;
												}
