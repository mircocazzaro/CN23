...
unsigned char  rttmopt[12] = { 0x08,0x0a, 0x00, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
unsigned char  mss_rttm[16] = { 0x02,0x04,0x05,0x90,0x08,0x0a, 0x00, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
switch(tcb->st){
    ...
    tcb->cong_st = SLOW_START;
         ...
         mss_rttm[9] = * (unsigned char *) now + 3;
         mss_rttm[8] = * (unsigned char *) now + 2;
         mss_rttm[7] = * (unsigned char *) now + 1;
         mss_rttm[6] = * (unsigned char *) now;
         
			prepare_tcp(s,SYN,NULL,0,mss_rttm,sizeof(mss_rttm));
			tcb->st = SYN_SENT;	
	...
	case SYN_SENT:	
		if(event == PKT_RCV){
            ...
            if ((tcp->d_offs_res >> 4) > 5) {
               if (tcp->payload[0] == 8) {
                  first_time = (tcp->payload[2] << 24) + (tcp->payload[3] << 16) + (tcp->payload[4] << 8) + (tcp->payload[5]);
                  second_time = (tcp->payload[6] << 24) + (tcp->payload[7] << 16) + (tcp->payload[8] << 8) + (tcp->payload[9]);
               }
               else if (tcp->payload[4] == 8) {
                  first_time = (tcp->payload[6] << 24) + (tcp->payload[7] << 16) + (tcp->payload[8] << 8) + (tcp->payload[9]);
                  second_time = (tcp->payload[10] << 24) + (tcp->payload[11] << 16) + (tcp->payload[12] << 8) + (tcp->payload[13]);
               }
            }

            now = (unsigned int) time(NULL);
            rttmopt[5] = * (unsigned char *) now + 3;
            rttmopt[4] = * (unsigned char *) now + 2;
            rttmopt[3] = * (unsigned char *) now + 1;
            rttmopt[2] = * (unsigned char *) now;
            
            rttmopt[9] = * (unsigned char *) first_time + 3;
            rttmopt[8] = * (unsigned char *) first_time + 2;
            rttmopt[7] = * (unsigned char *) first_time + 1;
            rttmopt[6] = * (unsigned char *) first_time;
             
            prepare_tcp(s,ACK,NULL,0,rttmopt,12);	
				tcb->st = ESTABLISHED;
				}
			}
		break;
