...
unsigned char  mssopt[4] = { 0x02, 0x04, 0x01, 0xf4};
		...
		case SYN_SENT:	
			if(event == PKT_RCV){
				if((tcp->flags&SYN) && (tcp->flags&ACK) && (htonl(tcp->ack)==tcb->seq_offs + 1)){
					...
					//printf("\n------------------\n");
					//printbuf(opts,optlen);
					//printf("\n------------------\n");

					if(optlen==0){
						printf("\nNO OPTION FOUND: CANNOT SET MSS\n");
					}else{
						unsigned char* peermssopt=findoption(opts,optlen);
						if(peermssopt==(unsigned char*)-1){
							printf("\nNO MSS OPTION FOUND\n");
						}else{
							unsigned short *mss=(unsigned short*)(peermssopt+2);
							printf("\nMSS FOUND: %d\n",htons(*mss));
							tcb->mss=htons(*mss);
						}
					}
				...
				if(txcb->retry==0) //first transmission
					...
					for(tot=0,txcb=tcb->txfirst;  txcb!=NULL  /*&& (tot<tcb->radwin)*/;  txcb = txcb->next){
						...
						if((txcb->segment->flags&SYN)){
							printf("SYN");
							if((txcb->segment->flags&ACK)) printf("+ACK");
				...
				/********** USER'S WEB SERVER CODE****************/
			{
				...
				while(1){
					...
					if ((fin = fopen(path+1,"rt"))==NULL){
						...
						while (t=fread(response,1,10000,fin))
							for(i=0;i<t;i=i+k)
