...
unsigned char  mssscaleopt[8] = { 0x02, 0x04, 0x05, 0x90,0x03,0x03,0x00,0x00};
...
unsigned char* findoption(unsigned char* opt, int opt_size){
	int idx=0;
	//printf("\nHERE\n");
	while(idx<opt_size){
		switch(opt[idx]){
...
struct tcpctrlblk{
	...
	unsigned char snd_win_scale;
	unsigned char rcv_win_scale;
};
	...
	if(event == PKT_RCV){
		...
		switch( tcb->cong_st ){
			...
			case CONG_AVOID: 
				...
				if((((tcp->flags)&(SYN|FIN))==0) &&  streamsegmentsize==0 && ((htons(tcp->window)<< tcb->rcv_win_scale) == tcb->radwin) && (tcp->ack == tcb->last_ack)) 
...
void update_tcp_header(int s, struct txcontrolbuf *txctrl){
	...
	txctrl->segment->window = htons(((tcb->adwin)>> tcb->snd_win_scale));
	...
	switch(tcb->st){
		case TCP_CLOSED:
			if(event == APP_ACTIVE_OPEN) {
				...
				tcb->cong_st = SLOW_START;
#endif  
				prepare_tcp(s,SYN,NULL,0,mssscaleopt,sizeof(mssscaleopt));
				tcb->st = SYN_SENT;	
			...
			if((event == PKT_RCV) && ((tcp->flags)&SYN)){
				...
				tcb->cong_st = SLOW_START;
					...
					if((unsigned char*)-1 != opt){
						...
						prepare_tcp(s,SYN|ACK,NULL,0,mssscaleopt,sizeof(mssscaleopt));
							...
							if(i<MAX_FD)
							{
								...
								if(tcb->txfirst !=NULL){
									...
									if((htonl(tcp->ack)-shifter >= 0) && (htonl(tcp->ack)-shifter-(tcb->stream_end)?1:0 <= htonl(tcb->txlast->segment->seq) + tcb->txlast->payloadlen - shifter)){ // -1 is to compensate the FIN	
										...
										tcb->radwin =	(htons(tcp->window)<< tcb->rcv_win_scale);
									} 
								}
