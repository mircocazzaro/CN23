...
unsigned char* findoption(unsigned char* opt, unsigned int opt_size) {
   int i = 0;
   while(i<opt_size) {
      switch(opt[i]) {
         case 0:
	...
	case SYN_SENT:	
		if(event == PKT_RCV){
            ...
            if((unsigned char*)-1 != opt) {
               ...
               printf("SYNACK ## WINDOW SIZE: %u * 2^%u = %u ##\n", ntohs(tcp->window), opt[2], winsize);
            } else {
            ...
            for(i=0;i<MAX_FD;i++)
			  ...
			  if(i<MAX_FD)
                  ...
                  /*else {
                     ...
                     opt = findoption(opt, opt_len);
                     if((unsigned char*)-1 != opt) {
                        printf("TCP FLAGS: %u ## WINDOW SCALE OPT FOUND. Value: %u ##\n", tcp->flags, opt[2]);
                     } else {
                        printf("TCP FLAGS: %u ## NO OPTION FOUND ##\n", tcp->flags);
                     }
