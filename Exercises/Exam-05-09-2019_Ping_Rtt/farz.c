...
int forge_icmp(struct icmp_packet * icmp){
	...
	unsigned int millis=(time.tv_sec*1000)+(time.tv_usec/1000);	
	printf("\n TIME: %d\n",millis);
	icmp->originate_time=htonl(millis);
	icmp->receive_time=0;
	icmp->transmit_time=0;
	icmp->checksum=htons(checksum(icmp,20));
	return 20;
}
...
int main(){
	...
	unsigned char destip[4]={90,147,80,34};
	...
	t = sendto(s, buf, 14+20+20, 0 ,(struct sockaddr *) &sll, len);
	if ( t == -1) {perror("sendto Failed"); return 1;}
	printf("%d bytes sent\n",t);
	while(1){
