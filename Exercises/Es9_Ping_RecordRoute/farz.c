...
void forge_ip(struct ip_datagram * ip, unsigned char* dst,  unsigned short payloadlen, unsigned char proto  )
{
	...
	ip->checksum = htons(checksum(ip,20+40));
}
void forge_eth( struct ethernet_frame * eth, unsigned char *dst, unsigned short type   )
{
...
int main(){
	int t,len;
	unsigned char destip[4] = {88,80,187,6};
	//unsigned char destip[4] = {147,162,2,100};
	//unsigned char destip[4] = {212,71,253,5};
	...
	printbuf(buf, 14 + 20+40 + 40 );
	t = sendto(s, buf, 14+20+40+40, 0 ,(struct sockaddr *) &sll, len);
	if ( t == -1) {perror("sendto Failed"); return 1;}
	printf("%d bytes sent\n",t);
	while(1){
	...
	//	printf("HERE");
		if(htons(eth->type)==0x0800) {
			//printf("IP received");
			if(ip->proto == 1){
				printf("\nICMP received %d %.2x\n",icmp->type,ip->ver_ihl);
				printbuf(buf,t);
				
				if(icmp->type == 0 && ip->ver_ihl==0x4f ){
					printf("Received Ping Response:\n");
					printbuf(buf,t);
					break;

				}else if( (((char *)ip)+20)[0]==12 ){
					printbuf(buf,t);
				}
			} 
		}
